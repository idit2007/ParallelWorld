...
public class ZembieNormolMove : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (PlayWorld== World)
        {
             ...
             if(Math.Sqrt(Math.Pow(Convert.ToDouble(tileX- PlaytileX),2)+ Math.Pow(Convert.ToDouble(tileY - PlaytileY), 2) ) < SeenRadius && TurnController.Instance.playerMovemnet ==true)
            {
